...
function nombreCompletoPromotor(
  p?:
    | {
...
export async function listGestiones(): Promise<GestionPagoRow[]> {
  const reservas = await must<Reserva[]>(
    (supabase as any)
      .from("reservas")
      .select(
        "id, cuh_id, promotor_id, cliente_id, estado, required_amount, expires_at, created_at, updated_at"
      )
  ...
  const byProm: Record<string, any> = Object.fromEntries((promosBatch || []).map((p: any) => [p.id, p]));
  ...
  for (const r of reservas) {
    ...
    labelUbicacion(
      propiedad.ubicacion,
      (propiedad.ubicacion || "").toUpperCase().includes("ESQUINA")
    );

    const cliente_doc = cliente.documento_identidad || cliente.full_phone || cliente.email || cliente.id;
...
/* ====== Soporte dinámico para columna `stage` en payments ====== */
let STAGE_SUPPORTED: boolean | null = null;

async function ensureStageSupport(): Promise<boolean> {
  if (STAGE_SUPPORTED !== null) return STAGE_SUPPORTED;
  try {
    const { error } = await (supabase as any).from("payments").select("stage").limit(1);
...
export async function listPagos(reservaId: string, stage?: PagoStage): Promise<Pago[]> {
  ...
  if (supported) {
    ...
    const pagos: Pago[] = (data || []).map((p: any) => ({
      id: p.id,
    ...
    return stage && stage !== "reserva" ? [] : pagos;
  }
}

-- ============================================================
-- HABITAD 2.0 — Padding LPAD para ubigeos de 5 dígitos
-- Fecha: 2026-05-29
-- ============================================================
-- Detectado en auditoría #3: 351 filas del staging tienen ubigeo
-- de 5 dígitos (códigos INEI sin el 0 inicial). 65 únicos matchean
-- contra v2.ubigeos al hacer LPAD a 6. Sin esto, ~22% de clientes
-- quedan sin ubigeo válido → SAP Maestro sin ZipCode/Block/City/County.
-- ============================================================

-- Backfill: actualiza clientes que tienen ubigeo NULL pero el staging
-- de su DNI tenía un ubigeo de 5 dig paddable.
UPDATE v2.clientes c
SET ubigeo_cod = u.codigo
FROM v2.import_cuh_staging s
JOIN v2.ubigeos u ON u.codigo = lpad(btrim(s.ubigeo_cod), 6, '0')
WHERE c.dni = btrim(s.dni)
  AND c.ubigeo_cod IS NULL
  AND length(btrim(s.ubigeo_cod)) = 5;

-- Actualiza fn_promote_staging para que aplique LPAD por defecto.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      IF r.dni IS NULL OR btrim(r.dni) = '' OR upper(btrim(r.dni)) = 'BLOQUEADO' THEN
        IF v_iter_warn THEN
          UPDATE v2.import_cuh_staging
            SET status='warning'::v2.import_status, warning='Etapa fuera de rango 1-21: ' || v_etapa_codigo,
                propiedad_id=v_propiedad_id, promoted_at=now(), error=NULL
            WHERE id = r.id;
          v_warn := v_warn + 1;
        ELSE
          UPDATE v2.import_cuh_staging
            SET status='promoted'::v2.import_status,
                propiedad_id=v_propiedad_id, promoted_at=now(), error=NULL
            WHERE id = r.id;
        END IF;
      ...
      -- Ubigeo con padding: matchea raw o lpad(6,'0')
      v_ubigeo_valid := NULL;
      ...
      ELSIF v_ubigeo_valid IS NOT NULL THEN
        -- Backfill ubigeo si el cliente lo tenía NULL
      ...
      IF v_venta_id IS NULL THEN
        ...
        INSERT INTO v2.ventas (
          ...
          v_precio, v_moneda,
          v_fecha_sep, v_fecha_sep + interval '24 hours',
      ...
      IF v_iter_warn OR (v_promotor_id IS NULL AND v_promotor_base IS NOT NULL) THEN
        UPDATE v2.import_cuh_staging SET
          status='warning'::v2.import_status,
          ...
          propiedad_id=v_propiedad_id, cliente_id=v_cliente_id, venta_id=v_venta_id,
          promoted_at=now(), error=NULL
        WHERE id = r.id;
        v_warn := v_warn + 1;
      ELSE
        UPDATE v2.import_cuh_staging SET
          status='promoted'::v2.import_status,
          propiedad_id=v_propiedad_id, cliente_id=v_cliente_id, venta_id=v_venta_id,
          promoted_at=now(), error=NULL
        WHERE id = r.id;
      END IF;

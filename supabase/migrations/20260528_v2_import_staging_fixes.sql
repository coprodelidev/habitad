-- ============================================================
-- HABITAD 2.0 — Fixes sobre fn_promote_staging
-- Fecha: 2026-05-28
-- ============================================================
-- Issues detectados en el primer smoke import (batch 39a00904…):
--   1) 622 errores por FK clientes_ubigeo_cod_fkey: el ubigeo del CSV
--      no siempre matchea v2.ubigeos. Solución: validar antes de insertar
--      y dejar NULL si no existe (no es campo crítico para Fase 0).
--   2) 46 errores por cast de num_cuotas con valores "OK"/"NO"/"PEND":
--      parsear con fn_parse_numeric, NULL si no es entero.
--   3) 108 "cuh vacío" se mantienen como error (filas inválidas reales).
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Ubigeo: validar contra v2.ubigeos. Si no existe → NULL (no romper).
      v_ubigeo_valid := NULL;
      ...
      -- num_cuotas tolerante: parse, si no es entero → NULL.

-- ============================================================
-- HABITAD 2.0 — Fix de cast enum + contadores rollback-safe
-- Fecha: 2026-05-29
-- ============================================================
-- Bugs detectados en tercera auditoría:
--   1. fn_promote_staging falla con "column status is of type
--      import_status but expression is of type text" en filas raras
--      donde el CASE de UPDATE devuelve text sin cast explícito.
--      Solución: cast a v2.import_status en todos los SET status=...
--   2. El contador propiedades_creadas/clientes_creados/ventas_creadas
--      se incrementa aunque la transacción de la fila rollbackee
--      (la EXCEPTION restaura savepoint pero las variables PL/pgSQL
--      no se revierten). Solución: contadores locales por iteración,
--      se suman al global solo si la iteración llega al final OK.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Solo sumar contadores si llegamos al final sin EXCEPTION
      v_props := v_props + v_props_local;
      v_clis := v_clis + v_clis_local;
      v_ventas := v_ventas + v_ventas_local;

    EXCEPTION WHEN OTHERS THEN
      -- Rollback automático del savepoint; NO sumamos los contadores locales
